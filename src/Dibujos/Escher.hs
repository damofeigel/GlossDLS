...
{-
esquina n x p | x == (n-1) = cuarteto p (lado n n p) (rotar (lado n n p)) (dibujoU p)
              | otherwise = cuarteto (esquina n (x+1) p) (lado n (x+1) p) (rotar (lado n (x+1) p)) (dibujoU p)
-}
...
{- 
lado n x p | x == n = cuarteto p p (rotar (dibujoT p)) (dibujoT p) 
lado n x p | x == n = cuarteto p p (rotar (dibujoT p)) (dibujoT p) 
           | otherwise = cuarteto (lado n (x+1) p) (lado n (x+1) p) (rotar (dibujoT p)) (dibujoT p) 
-}
...
{-
interpBasica :: Output Escher
interpBasica Triangulo a b c = line $ map (a V.+) [zero, c, b, zero]
-}
escherConf :: Conf
escherConf = Conf {
    name = "Escher",
    pic = interp interpBasica (escher 6 Triangulo)
}